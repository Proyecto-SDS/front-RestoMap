...
interface SidebarCocineroProps {
  activeItem: 'pedidos' | 'inventario';
  onNavigate: (item: 'pedidos' | 'inventario') => void;
}

export function SidebarCocinero({ activeItem, onNavigate }: SidebarCocineroProps) {
    ...
    <aside className="hidden lg:flex lg:flex-col lg:w-[260px] bg-white border-r border-[#E2E8F0] min-h-screen sticky top-0">
      ...
      <div className="p-6 border-b border-[#E2E8F0]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[#334155]">Panel Cocina</h2>
            <p className="text-xs text-[#94A3B8]">Cocinero</p>
          </div>
        </div>
      </div>
