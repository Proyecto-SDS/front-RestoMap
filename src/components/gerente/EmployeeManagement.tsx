...
export function EmployeeManagement({
  ...
  const roles = ['todos', 'admin', 'mesero', 'cocinero', 'bartender', 'reservas'];
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl text-[#334155] mb-1">Gestión de Empleados</h2>
                ...
                {paginatedEmpleados.map((empleado) => (
                  ...
                  >
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-linear-to-br from-[#F97316] to-[#EF4444] rounded-full flex items-center justify-center shrink-0">
                          <span className="text-white text-sm">{getInitials(empleado.nombre)}</span>
                        </div>
                        <span className="text-sm text-[#334155]">{empleado.nombre}</span>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <span className="text-sm text-[#64748B]">{empleado.correo}</span>
                    </td>
                    <td className="py-4 px-4">
                      <span
                    ...
                    <td className="py-4 px-4">
                      ...
                      >
                        {empleado.estado === 'activo' ? <UserCheck size={12} /> : <UserX size={12} />}
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          {empleado.estado === 'activo' ? <UserX size={16} /> : <UserCheck size={16} />}
                        </button>
                        <button
